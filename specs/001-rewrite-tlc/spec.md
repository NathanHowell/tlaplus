...
- Progress indicator must remain responsive when total state count is unknown or changes significantly during exploration by maintaining the ≤5 second refresh cadence defined in NFR-001.
...
- When a resume attempt encounters a checkpoint manifest from a mismatched binary or schema version, the tool MUST abort, reference the manifest path, and explain how to restart safely.
...
- **Checkpoint Version Guards**: Include negative resume tests that exercise mismatched checkpoint manifests and assert the CLI surfaces actionable restart guidance rather than proceeding with stale data.
- **Fingerprint Determinism Tests**: Capture canonicalized states and verify the resulting 128-bit fingerprints remain identical across repeated runs, architectures, and toolchain updates.
- **Toolbox Compatibility Sweep**: Execute Toolbox automation smoke tests against updated CLI outputs to confirm downstream integrations remain functional.
