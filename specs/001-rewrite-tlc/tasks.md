...
- [ ] T089 [US1] Implement TLA+ semantic evaluator for `Init`/`Next`/invariants in `src/engine/src/semantics.rs` — **Acceptance Criteria**: provide an API that returns initial states, successor states, and invariant predicates with structured errors; cover binding/substitution edge cases via new unit tests in `tests/unit/semantics.rs`; `cargo test -p tlc-engine` passes.
