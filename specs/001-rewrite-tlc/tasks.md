...
- [X] T087 [P] [US1] Implement TLA+ module parser and AST construction in `src/util/src/parser/mod.rs`
- [X] T088 [US1] Integrate TLA+ parser into ingestion validating module syntax and reportable errors in `src/cli/src/input_loader.rs`
- [X] T089 [US1] Implement TLA+ semantic evaluator for `Init`/`Next`/invariants in `src/engine/src/semantics.rs` — **Acceptance Criteria**: provide an API that returns initial states, successor states, and invariant predicates with structured errors; cover binding/substitution edge cases via new unit tests in `tests/unit/semantics.rs`; `cargo test -p tlc-engine` passes.
- [X] T090 [US1] Implement core exploration loop with fingerprint deduplication and invariant checking in `src/engine/src/explorer.rs` — **Acceptance Criteria**: produce run metrics and violations compatible with `RunMetricsRecorder`, fail fast on invariant breaches, and respect queue/scheduler back pressure; add unit tests in `tests/unit/explorer.rs` exercising depth-first and breadth-first traversal; `cargo test -p tlc-engine` passes.
- [X] T091 [US1] Add end-to-end model checker regression covering safe/unsafe specs in `tests/integration/model_checker_basic.rs` — **Acceptance Criteria**: include fixtures that (a) terminate cleanly with zero violations and (b) surface a specific invariant failure with matching CLI output; wire test into `cargo test -p tlc-integration-tests`; ensure run summaries include explored state counts and fingerprint totals.
